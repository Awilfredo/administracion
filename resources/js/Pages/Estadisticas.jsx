...
function Estadisticas({ data, auth }) {
    ...
    function obtenerTimestampsMes(year, month) {
        ...
        for (let day = 1; day <= daysInMonth(year, month); day++) {
            ...
            const fechaHoraISO = `${year}-${formatTwoDigits(month + 1)}-${formatTwoDigits(day)}T00:00:00`;
            ...
            const fechaHoraFormateada = `${fechaHoraObj.getFullYear()}-${formatTwoDigits(fechaHoraObj.getMonth() + 1)}-${formatTwoDigits(fechaHoraObj.getDate())} ${formatTwoDigits(fechaHoraObj.getHours())}:${formatTwoDigits(fechaHoraObj.getMinutes())}:${formatTwoDigits(fechaHoraObj.getSeconds())}.${formatMilliseconds(fechaHoraObj.getMilliseconds())}${getTimezoneOffset(fechaHoraObj)}`;
    ...
    function getTimezoneOffset(date) {
        ...
        return `${offsetSign}${formatTwoDigits(offsetHours)}${formatTwoDigits(offsetMinutesAbs)}`;
    }
    ...
    useEffect(() => {
        ...
        timestampsJunio2024.forEach((e) => {
            ...
            if(suma_elementos > 0){
                dia_elemnto.horas=(suma_horas/suma_elementos)/(60);
                datos_grafica.push({name:dia_elemnto.name, horas:dia_elemnto.horas})
            }
